...
- **Image** — a _required free-form input_ question for the image URL. Tell the user to right-click the Shopee product photo and copy the image address (`https://down-tw.img.susercontent.com/file/<hash>`).
  You may suggest a default value in a question, but never select, assign, or write that value without the user's explicit answer. Do not treat dismissing a question, skipping it, or submitting a blank/whitespace-only answer as consent. Re-ask that same question until the user supplies a non-empty answer. Batch the initial four questions into a single `#tool:vscode_askQuestions` call; follow-up questions may be separate calls.

3. Show the complete drafted object to the user, then use `#tool:vscode_askQuestions` to request approval before writing. The approval question must offer exactly `Confirm and add` and `Cancel`; do not ask for approval in plain chat text. Write the entry only when the user explicitly selects `Confirm and add`. If the user selects `Cancel`, dismisses the question, or gives no explicit approval, stop without editing any file.
...
- Always request final approval through `#tool:vscode_askQuestions` after showing the complete drafted object. Never treat displaying the draft, a prior field answer, or a plain-chat reply as approval.
- Never edit [products.ts](../../src/data/products.ts) or [categories.ts](../../src/data/categories.ts) until the user explicitly selects `Confirm and add` in the approval question. Cancellation, dismissal, or an ambiguous response must leave files unchanged.
